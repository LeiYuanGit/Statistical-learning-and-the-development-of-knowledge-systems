...
data_n_trial_level = data_n_trial_level_raw %>%
  select(id, sex, ses, age_months, location, acc, completed_task, within_id) %>%
  mutate(task = "N")
...
data_n_subj_level = data_n_trial_level %>%
  group_by(id, age_months, sex, ses, location, task, completed_task, within_id) %>%
...
total_n_trials = nrow(data_n_trial_level) + nrow(data_more_trial_level)
...
subj_one_task = data_combined_subj_level %>%
  ...
  select(within_id, age_months, sex, location)

subj_two_tasks = data_combined_subj_level %>%
  filter(completed_task == 2) %>%
  group_by(within_id, age_months, sex, location) %>%
  summarise(n = n()) %>%
  select(-n)
